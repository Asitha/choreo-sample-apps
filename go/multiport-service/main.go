...
func hello8080(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintf(w, "Hello %s, from port 8080", r.URL.Query().Get("name"))
}

func hello6657(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintf(w, "Hello %s, from port 6657", r.URL.Query().Get("name"))
}

func hello9876(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintf(w, "Hello %s, from port 9876", r.URL.Query().Get("name"))
}

func hello9090(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintf(w, "Hello %s, from port 9090", r.URL.Query().Get("name"))
}
